...
# program constants - CHANGE THESE TO MATCH SPLAT.PHYSICS.UCSD.EDU
BASE_FOLDER = '/var/www/html/splat/archive/'
TEMPLATE_FOLDER = BASE_FOLDER+'/templates/'
PLOT_FOLDER = BASE_FOLDER+'/review_plots/'
DATABASE_FOLDER = BASE_FOLDER+'/databases/'
COMBSPEC_FOLDER = BASE_FOLDER+'/combspec/'
DATA_FOLDER = BASE_FOLDER+'/data/'
LXD_DATA_FOLDER = DATA_FOLDER+'/lxd/'
PRISM_DATA_FOLDER = DATA_FOLDER+'/prism/'
SXD_DATA_FOLDER = DATA_FOLDER+'/sxd/'


PRISM_TEMPLATE_FILE = TEMPLATE_FOLDER+'/prism_template.txt'
SXD_TEMPLATE_FILE = TEMPLATE_FOLDER+'/sxd_template.txt'
PAGE_TEMPLATE_FILE = TEMPLATE_FOLDER+'/page_template.txt'
DATE_TEMPLATE_FILE = TEMPLATE_FOLDER+'/date_template.txt'
HTML_OUTPUT_NAME = BASE_FOLDER+'/index.html'

csv_headers = {
    'NAME': 'NAME',
    'DESIGNATION': 'OBSERVED_DESIGNATION',
    'DATE': 'UT_DATE',
    'UT TIME': 'UT_TIME',
    'OBSERVER': 'OBSERVER',
    'PROGRAM': 'program_info',
    'REDUCTION DATE': 'REDUCTION_DATE',
    'REDUCER': 'REDUCTION_PERSON',
    'DATA FILE': 'DATA FILE',
    'INSTRUMENT': 'GRATING',
}
other_headers = {
...
def check_files(files, keyword):
    ...
    for y in range(len(files)):
        fits_name = files[y].split(keyword + "/")[1]
        file_dir = files[y].split(keyword + "/")[0]
        ...
        if re.search('spex-' + keyword, split_file[0]) is None:
            if split_file[0] == 'spex' and split_file[1] == keyword:
                ...
                try:
                    os.rename(files[y], file_dir + keyword + '/' + prop_name)
                    obj_list[y] = file_dir + keyword + '/' + prop_name
                    continue
...
# MAIN PAGE-MAKING FUNCTION
def makePage(debug=True):
# first read in templates
# page
    f = open(PAGE_TEMPLATE_FILE,'r')
    txt = f.read()
    f.close()
    ptop,pbot = txt.split('[SPLIT]')
# date
    f = open(DATE_TEMPLATE_FILE,'r')
    txt = f.read()
    f.close()
    dtop,dbot = txt.split('[SPLIT]')
# prism
    f = open(PRISM_TEMPLATE_FILE,'r')
    ptemp = f.read()
    f.close()
# sxd
    f = open(SXD_TEMPLATE_FILE,'r')
    stemp = f.read()
    f.close()

    spcsv = glob.glob(DATABASE_FOLDER+'/spectra*.csv')
    spcsv.sort()
    if len(spcsv)==0:
        print('No spectra*.csv files in {}; not producing a webpage'.format(DATABASE_FOLDER))
        return

    output = ptop
    
    dates = [c.split('_')[-2] for c in spcsv]
    instruments = [(c.split('_')[-1]).replace('.csv','').lower() for c in spcsv]
    for i,date in enumerate(dates):
        output=output+dtop.replace('[DATE]',date)
        db = pd.read_csv(spcsv[i])
        db.sort_values('UT_TIME',inplace=True)
        if debug==True: print(date,db.columns,instruments[i])

# temporary fix for different column names
        dref = ''
        for k in ['File_Name','DATA FILE','DATA_FILE']: 
            if k in list(db.columns): dref = k

# prism            
        if instruments[i].lower()=='prism':
            for j,file in enumerate(db[dref]):
                if str(file) == 'nan':
                    continue
                temp=copy.deepcopy(ptemp)
                for nm in list(csv_headers.keys()):
                    if csv_headers[nm] in list(db.columns):
                        temp = temp.replace('[{}]'.format(nm),str(db[csv_headers[nm]].iloc[j]))
# plots
                ref = file.replace('.fits','')
                srch = PLOT_FOLDER+'/plot_{}.png'.format(ref)
                pfile = glob.glob(srch)
                if len(pfile) == 0: print('Could not find review plot {}'.format(srch))
                else: temp = temp.replace('[PLOT]',pfile[0])
                srch = PLOT_FOLDER+'/plot-classify_{}.png'.format(ref)
                pfile = glob.glob(srch)
                if len(pfile) == 0: print('Could not find review plot {}'.format(srch))
                else: temp = temp.replace('[CLASSIFICATION PLOT]',pfile[0])
# fits
#                srch = PRISM_DATA_FOLDER+'/{}'.format(db[csv_headers['DATA FILE']].iloc[j])
#                pfile = glob.glob(srch)
#                if len(pfile) == 0: print('Could not find fits file {}'.format(srch))
#                else: temp = temp.replace('[DATA FILE]',pfile[0])
# other replacements
                temp = temp.replace('[SNR]','{:.0f}'.format(db[other_headers['SNR']].iloc[j]))
                temp = temp.replace('[CLASSIFICATION]','N/A')
                output=output+temp

# SXD            
        elif instruments[i].lower()=='sxd':
            for j,file in enumerate(db[dref]):
                if str(file) == 'nan':
                    print("Yes")
                    continue
                temp=copy.deepcopy(stemp)
                for nm in list(csv_headers.keys()):
                    if csv_headers[nm] in list(db.columns):
                        temp = temp.replace('[{}]'.format(nm),str(db[csv_headers[nm]].iloc[j]))
# plots
                if debug==True: print(j,file)
                ref = file.replace('.fits','')
                srch = PLOT_FOLDER+'/plot-orders_{}.png'.format(ref)
                pfile = glob.glob(srch)
                if len(pfile) == 0: print('Could not find orders review plot {}'.format(srch))
                else: temp = temp.replace('[ORDERS PLOT]',pfile[0])
                srch = PLOT_FOLDER+'/plot_{}.png'.format(ref)
                pfile = glob.glob(srch)
                if len(pfile) == 0: print('Could not find review plot {}'.format(srch))
                else: temp = temp.replace('[PLOT]',pfile[0])
...
# merged fits file
                # srch = '{}/{}'.format(SXD_DATA_FOLDER,db[csv_headers['DATA FILE']].iloc[j])
                # pfile = glob.glob(srch)
                # if len(pfile) == 0: print('Could not find fits file {}'.format(srch))
                # else: temp = temp.replace('[DATA FILE]',pfile[0])

                srch = (file.split('/')[-1]).replace('sxd','sxd-merged')
                srch = '{}/{}'.format(SXD_DATA_FOLDER,srch)
                pfile = glob.glob(srch)
                if len(pfile) == 0: print('Could not find merged fits file {}'.format(srch))
                else: temp = temp.replace('[MERGED DATA FILE]',pfile[0])
...
#
# INSERT LXD STATEMENT HERE
#

        else: 
            raise ValueError('Do not recognize instrument {} for date {}'.format(instruments[i],date))
        output=output+dbot
    output=output+pbot
    
# move html file if necessary
    if os.path.exists(HTML_OUTPUT_NAME):
        shutil.copy(HTML_OUTPUT_NAME,HTML_OUTPUT_NAME.replace('.html','_old.html'))
...
if __name__ == '__main__':
	...
	for x in list(file_type_prefix):
		if x == 'com':
			...
			copy_files(pathway, 'com', '.fits', COMBSPEC_FOLDER)
            continue
		if x == 'lxd':
...
#		print(pathway)
		try:
			file_check = search_files(destination_folders[x], x, '.fits')
			check_files(file_check, x)            
			copy_files(pathway, x, '.fits', destination_folders[x])
			copy_files(pathway, x, '.fits', DATA_FOLDER + x + '/')
		except:
			print("Error copying fits files over.")
