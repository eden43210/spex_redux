...
def data_sheet(path, file_type = 'spex_prism'):
    ...
    assert any(list(file_type_prefix.keys())[x] == file_type for x in range(len(list(file_type_prefix.keys()))))== True, 'Given file type does not exist'
    
    #Search for spex_prisim files
    files = glob.glob(path+ f'/{file_type}*.fits')  #Remove _prism_ to also be able to include sxd_merged
...
#    print(file_type_prefix[file_type])
    date_taken = date_taken.format(UT = spreadsheet['UT_DATE'][0], FT = file_type_prefix[file_type])
...
#            date_taken = "/sources_{UT}.csv"
        date_taken = date_taken.format(UT = data['UT_DATE'][0], FT = file_type_prefix[file_type])
        #Set index = False to get rid of the numbered column at the very beginning.
...
#    files = glob.glob(folder+ f'/{file_type}*.fits')  #Remove _prism_ to also be able to include sxd_merged
#    files = [f for f in files if 'tellspec' not in f]
#    files = [f for f in files if 'modVega' not in f]
...
#    files = glob.glob(folder+ f'/{file_type}*.fits')
#    files = [f for f in files if 'tellspec' not in f]
#    files = [f for f in files if 'modVega' not in f]
...
# classify prism files
    ...
    if len(files) == 0: print('Cannot find any {}*.fits data files in {}'.format(file_type, folder))
        ...
        for i,m in enumerate(files): 
            ...
            splat.classifyByStandard(sp,plot=True,method='kirkpatrick',telluric=True,output=folder+'/plot_classify_{}_{}_{}.pdf'.format(names[i],dates[i], file_type_prefix[file_type]))
...
if __name__ == '__main__':
	if len(sys.argv) > 1:
		for x in list(file_type_prefix.keys()):          
			process(sys.argv[1], file_type = x)
			data_sheet(sys.argv[1], file_type = x)
#			print(data_sheet(path = sys.argv[1], file_type = x))  
			first = info_tab(data_sheet(path = sys.argv[1], file_type = x), catalogs_given = cats_list1, up_coord_flag = True, file_type = x)
			info_tab(data = first, catalogs_given = cats_list2, up_coord_flag = False, file_type = x)
            
#		process(sys.argv[1], file_type = sys.argv[2])
#		data_sheet(sys.argv[1], file_type = sys.argv[2])
#		first = info_tab(data_sheet(path = sys.argv[1], file_type = sys.argv[2]), catalogs_given = cats_list1, up_coord_flag = True)
