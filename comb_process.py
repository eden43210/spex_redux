...
#    assert any(file_type_prefix[x] == file_type for x in range(len(file_type_prefix)))== True, 'Given file type does not exist'
    ...
    for y in range(len(file_list)):
        if file_type in file_list[y]:
            if '.pdf' in file_list[y] or '.png' in file_list[y] or '.csv' in file_list[y]:
                continue
...
if __name__ == '__main__':
	for x in list(file_type_prefix):
		if x == 'com':
			continue               #For now, just ignore combspec files until first phase is finished
		if x == 'lxd':
