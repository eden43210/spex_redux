...
#                    continue
    ...
    for i in range(len(files)):
        ...
        for x in data_non_head:
            ...
            if x == 'tell_list':
                if 'notelluric' in files[i] or 'com_spec' in files[i]:
...
# plot individual SXD orders
        ...
        if len(files_ind) == 0: print('Cannot find any spex-sxd*.fits data files in {}'.format(folder))
            ...
            for i in range(len(files_ind)): 
                ...
                plt.savefig(folder+'/plot_orders_{}_{}_sxd.png'.format(names[i],dates[i]))
...
# classify merged SXD files
        ...
        if len(files_merged) == 0: print('Cannot find any spex-sxd-merged*.fits data files in {}'.format(folder))
            ...
            for i,m in enumerate(files_merged): 
                ...
                plt.savefig(folder+'/plot_classify_{}_{}_sxd.png'.format(names[i],dates[i]))
                sp.plot(file='plot_{}_{}_sxd.pdf'.format(names[i], dates[i]),telluric=True)
                plt.savefig(folder+'/plot_{}_{}_sxd.png'.format(names[i],dates[i]))
...
# classify prism files
        if len(files) == 0: print('Cannot find any {}*.fits data files in {}'.format(file_type, folder))
            ...
            for i,m in enumerate(files): 
                ...
                plt.savefig(folder+'/plot_classify_{}_{}_prism.png'.format(names[i],dates[i]))
                sp.plot(file='plot_{}_{}_prism.pdf'.format(names[i],dates[i]),telluric=True)
                plt.savefig(folder+'/plot_{}_{}_prism.png'.format(names[i],dates[i]))

    return

def check_files(files, keyword):
    #The purpose of this function is to search through a list of files and check for mistakes in spelling.  If the mistake can be identified, the code will 
    # change it.  If it can't be identified, the user will be asked to correct the mistake.
    objects = ['unchecked'] * len(files)
    obj_list = list(files)
#    print(files)
    ...
    for y in range(len(files)):
        ...
        if len(split_file) > 4:
            ...
            print("Please carefully check the file for any additional '_' values and post the correct name below")
        ...
        if len(split_file) == 4:
            if split_file[0] == 'spex' and split_file[1] == keyword:
                print("There was a naming issue with the following file:" + fits_name)
                prop_name = split_file[0] + '-' + split_file[1] + '_' + split_file[2] + '_' + split_file[3]
                os.rename(files[y], file_dir + 'proc/' + prop_name)
                obj_list[y] = file_dir + 'proc/' + prop_name
            else:
                print("There was a naming issue with the following file:" + fits_name)
                ...
                obj_list[y] = file_dir + 'proc/' + prop_name
        if len(split_file) == 3:
            #This will probably have to be updated
            objects[y] = 'checked'
        if len(split_file) < 3:
            print("There was a naming issue with the following file:" + fits_name)
            print("Please carefully check the file for any missing '_' values and post the correct name below")
            prop_name = input("Correct name:")
            print("Attempting file check with the following file name: " + prop_name)
            os.rename(files[y], file_dir + 'proc/' + prop_name)
            obj_list[y] = file_dir + 'proc/' + prop_name
...
#    print(objects)
    if any(objects[x] == 'unchecked' for x in range(len(objects))) == True:
        check_files(obj_list, keyword)
    else:
        print("All objects have the correct number of underscores")
...
def check_coord(text):
    #First, this function needs to determine if the object was named using a numeric format or a non-numeric format.  The best way to determine this would be
    # to check if there are any letters in the coordinates aside from a J at the beginning.  If there are, then it is non-numeric, otherwise, it is numeric.
    if len(re.findall(r'[a-zA-Z]', text)) > 1:
        text_type = 'non_numeric'
    if len(re.findall(r'[a-zA-Z]', text)) <= 1:
        text_type = 'numeric'
    
#    print(text_type)
    if text_type == 'numeric' and re.search('^J', text) is None:
        print("J missing")
        text = re.sub(r'([0-9]{4})', r'J\1', text, 1)
    if text_type == 'numeric' and len(re.findall('[0-9]{4}', text)) != 2:
        print("Coordinates mistaken for the following file: " + text)
        print("Please enter the correct coordinates below")
        text = input("Correct coordinates:")
    if text_type == 'numeric' and re.search('[0-9](\W)[0-9]', text) is None:
        print("Missing + or - in coordinates for the following file: " + text)
        print("Please enter the correct coordinates below")
        text = input("Correct coordinates:")
    if text_type == 'numeric' and len(text) > 10:
        print("Too many characters in coordinates for the following file: " + text)
        print("Please enter the correct coordinates below")
        text = input("Correct coordinates:")
    if text_type == 'numeric' and len(text) < 10:
        print("Too few characters in coordinates for the following file: " + text)
        print("Please enter the correct coordinates below")
        text = input("Correct coordinates:")
    return(text)
